...
public class LEC_FE_Utils
{
	...
	public static int getAuthorisedInvoicePL(int sri_authosisation_id) {
		...
		int c_invoice_authorised_id = DB.getSQLValue(null, "SELECT COALESCE(MAX(i.C_Invoice_ID), -1) FROM C_Invoice i WHERE i.SRI_AuthorizationPL_ID = ? ", sri_authosisation_id);
	...
	*/
	...
	public static int notifyUsers(Properties ctx, MMailText mText, int ad_user_id, String subject, 
			String message, List<File> attachments,  String trxName, int ad_user2_id)
	{
		int countMail = 0;
