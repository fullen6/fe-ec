...
public class LEC_FE_ModelValidator extends AbstractEventHandler {
	...
	protected void initialize() {
		...
		registerTableEvent(IEventTopics.PO_BEFORE_NEW, MInOut.Table_Name);
		registerTableEvent(IEventTopics.PO_AFTER_NEW, MInvoicePaySchedule.Table_Name);
	}
	...
	protected void doHandleEvent(Event event) {
		...
		if (po.get_TableName().equals(MOrder.Table_Name) && type.equals(IEventTopics.DOC_AFTER_COMPLETE)) {
			MOrder order = (MOrder) po;
			delpaymentprogram(order);
		}

		if (po.get_TableName().equals(MInOut.Table_Name) && type.equals(IEventTopics.DOC_BEFORE_COMPLETE)) {
			MInOut inout = ((MInOut) po);
		...
		if (po.get_TableName().equals(MMovement.Table_Name) && (type.equals(IEventTopics.DOC_BEFORE_COMPLETE))) {
		...
		if (po.get_TableName().equals(MOrderLine.Table_Name) && type.equals(IEventTopics.PO_AFTER_CHANGE)) {
		...
		if (po.get_TableName().equals(MInOut.Table_Name) && type.equals(IEventTopics.DOC_AFTER_PREPARE)) {
			log.severe("MInOut-----------------DOC_AFTER_PREPARE");
		...
		if (po.get_TableName().equals(MInvoicePaySchedule.Table_Name) && (type.equals(IEventTopics.PO_AFTER_NEW))) {

			MInvoicePaySchedule ps = (MInvoicePaySchedule) po;
			MInvoice inv = new MInvoice(ps.getCtx(), ps.getC_Invoice_ID(), ps.get_TrxName());
			ps.set_ValueOfColumn("LEC_PaymentMethod", inv.get_Value("LEC_PaymentMethod"));
			ps.saveEx();
		}
		...
		if (po.get_TableName().equals(MInvoiceLine.Table_Name) && (type.equals(IEventTopics.PO_BEFORE_NEW))) {
			...
			MOrderLine oline = new MOrderLine(invoiceline.getCtx(), invoiceline.getM_InOutLine().getC_OrderLine_ID(),
					invoiceline.get_TrxName());
		...
		if (po.get_TableName().equals(MInvoiceLine.Table_Name)
			...
			if (!shortdoctype.equals("")) {
				validateDigitAllowed(invoiceLine.getQtyEntered().doubleValue(), 6, MInvoiceLine.COLUMNNAME_QtyEntered);// Max
																														// 6
																														// digit
																														// on
				// factura_v1.1.0.xsd
				// SRI
				if (invoice.isSOTrx())
					validateDigitAllowed(invoiceLine.getPriceEntered().doubleValue(), 6,
							MInvoiceLine.COLUMNNAME_PriceEntered);// Max
				// 2
				// digit
				// on
				// factura_v1.1.0.xsd
		...
		if (po.get_TableName().equals(MOrderLine.Table_Name)
			...
			MDocType dt = new MDocType(order.getCtx(), order.getC_DocTypeTarget_ID(), order.get_TrxName());
			int dtinvId = 0;
			if (dt.get_Value("C_DocTypeInvoice_ID") != null) {
				if (Integer.valueOf(dt.get_Value("C_DocTypeInvoice_ID").toString()) > 0) {
					dtinvId = Integer.valueOf(dt.get_Value("C_DocTypeInvoice_ID").toString());
					MDocType dtinv = new MDocType(order.getCtx(), dtinvId, order.get_TrxName());
					String shortdoctype = dtinv.get_ValueAsString("SRI_ShortDocType");
					//
					if (!shortdoctype.equals("")) {
						validateDigitAllowed(orderLine.getQtyEntered().doubleValue(), 6,
								MOrderLine.COLUMNNAME_QtyEntered); // Max 6
						// digit on
						// factura_v1.1.0.xsd
						validateDigitAllowed(orderLine.getPriceEntered().doubleValue(), 6,
		...
		if (po.get_TableName().equals(MLCOInvoiceWithholding.Table_Name) && (type.equals(IEventTopics.PO_AFTER_NEW))) {
			...
			if (!invoice.isSOTrx()) {
				...
				keepWithholdingDocumentNoAfterGenerated(iwh, invoice, newWithholding, invoice.get_TrxName());
			}
	...
	private void delpaymentprogram(MOrder order) {
		...
		int result = DB.executeUpdate("DELETE FROM C_OrderPaySchedule WHERE C_Order_ID = ? ", order.get_ID(), true,
				order.get_TrxName());
		// log.warning("programa de pago borrado en la orden: "+order.getDocumentNo()+
		// result);
	}
	...
	private String validateMovement(MMovement movement) {// BeforeComplete
		...
		if (!(dt.get_Value("SRI_ShortDocType") == null) && !dt.get_ValueAsString("SRI_ShortDocType").isEmpty()) {
			...
			if (shortdoctype.equals("06")
					&& (movement.getDescription() == null || movement.getDescription().trim().length() == 0)) { // GUÍA
																												// DE
																												// REMISIÓN
			...
			if (dt.get_Value("SRI_ShortDocType") != null) {
				if (movement.getAD_User_ID() <= 0)
					msg = Msg.translate(Env.getCtx(), "FillMandatory") + " "
							+ Msg.getElement(Env.getCtx(), MMovement.COLUMNNAME_AD_User_ID);
				...
				MBPartner bp = MBPartner.get(Env.getCtx(), movement.getC_BPartner_ID());
				...
				if (movement.getDeliveryViaRule() == null)
					msg = Msg.translate(Env.getCtx(), "FillMandatory") + " "
							+ Msg.getElement(Env.getCtx(), MMovement.COLUMNNAME_DeliveryViaRule);
				...
				if (movement.getM_Shipper() == null)
					msg = Msg.translate(Env.getCtx(), "FillMandatory") + " "
							+ Msg.getElement(Env.getCtx(), MMovement.COLUMNNAME_M_Shipper_ID);
				if (movement.get_Value("ShipDate") == null)
					msg = Msg.translate(Env.getCtx(), "FillMandatory") + " " + Msg.getElement(Env.getCtx(), "ShipDate");
	...
	private void generateDiscount(MOrderLine oline) {
		...
		if (oline.get_ValueAsBoolean("IsDiscountApplied")) {
			BigDecimal calcdesc = DB.getSQLValueBD(oline.get_TrxName(),
					"SELECT sum(DiscountAmt) " + "FROM C_OrderLine WHERE C_Order_ID = ? ", oline.getC_Order_ID());
			...
			if (discount != null) {
				Integer LineOrder = DB.getSQLValue(
						oline.get_TrxName(), "SELECT C_OrderLine_ID FROM C_OrderLine WHERE C_Order_ID = ? "
								+ "AND C_Charge_ID IN " + " (Select C_Charge_ID FROM C_Charge WHERE isDiscountFE='Y') ",
						oline.getC_Order_ID());
				...
				if (LineOrder > 0) {
					...
					Integer C_ChargeID = DB.getSQLValue(null,
							" SELECT C_Charge_ID FROM C_Charge WHERE IsDiscountFE='Y' ");
					if (C_ChargeID <= 0)
						throw new AdempiereException(
								"Debe configurar un cargo para descuentos de Facturación Electrónica");
	...
	public static String valideOrgInfoSri(MOrgInfo orginfo) {
		...
		String clavecert = MSysConfig.getValue("QSSLEC_FE_ClaveCertificadoDigital", null, orginfo.getAD_Client_ID(),
				orginfo.getAD_Org_ID());

		if (clavecert == null)
			msg = "No existe parametro para ClaveCertificadoDigital";

		String rutacert = MSysConfig.getValue("QSSLEC_FE_RutaCertificadoDigital", null, orginfo.getAD_Client_ID(),
				orginfo.getAD_Org_ID());
		...
		if (rutacert != null) {
			...
			MAttachment attach = MAttachment.get(orginfo.getCtx(), MTable.getTable_ID("AD_Org"),
					orginfo.getAD_Org_ID());
			if (attach != null) {
			...
			if (dt.get_Value("SRI_ShortDocType") != null) {
				...
				if (dt.get_ValueAsString("SRI_ShortDocType").equals("05")
					...
					if (invoice.get_Value("SRI_RefInvoice_ID") == null
							&& invoice.get_Value("InvoiceDocumentReference") == null)
						msg = Msg.translate(Env.getCtx(), "FillMandatory") + " "
			...
			if (doctype.get_ValueAsString("SRI_ShortDocType") != null
				...
				if (inout.getAD_User().getEMail() == null)
					msg = Msg.translate(Env.getCtx(), "FillMandatory") + " "
							+ Msg.getElement(Env.getCtx(), MUser.COLUMNNAME_AD_User_ID) + " - "
							+ Msg.getElement(Env.getCtx(), MUser.COLUMNNAME_EMail);
				MBPartner bp = MBPartner.get(Env.getCtx(), inout.getC_BPartner_ID());
				if (bp.get_Value(X_LCO_TaxIdType.COLUMNNAME_LCO_TaxIdType_ID) == null)
					msg = Msg.translate(Env.getCtx(), "FillMandatory") + " "
							+ Msg.getElement(Env.getCtx(), MBPartner.COLUMNNAME_C_BPartner_ID) + " - "
							+ Msg.getElement(Env.getCtx(), X_LCO_TaxIdType.COLUMNNAME_LCO_TaxIdType_ID);
				if (!inout.getDeliveryViaRule().equals(MInOut.DELIVERYVIARULE_Shipper))
					msg = Msg.translate(Env.getCtx(), "FillMandatory") + " "
							+ Msg.getElement(Env.getCtx(), MInOut.COLUMNNAME_DeliveryViaRule);
				if (inout.getM_Shipper() == null)
					msg = Msg.translate(Env.getCtx(), "FillMandatory") + " "
							+ Msg.getElement(Env.getCtx(), MInOut.COLUMNNAME_M_Shipper_ID);
				if (inout.getShipDate() == null)
					msg = Msg.translate(Env.getCtx(), "FillMandatory") + " "
							+ Msg.getElement(Env.getCtx(), MInOut.COLUMNNAME_ShipDate);
	 ...
	 */
	private void validateDigitAllowed(double amt, int digitAllowed, String ColumnName) {
		...
		if ((decimal.length() - 1) > digitAllowed) { // .00 = 3 Digits
			throw new AdempiereException(
					Msg.translate(Env.getCtx(), "Maximo permitido " + digitAllowed + " digitos decimales") + " - "
	 ...
	 *
	 * @param MLCOInvoiceWithholding iwh,MInvoice invoice, boolean newWithholding,
	 *                               String trxName
	 *
	...
	private String keepWithholdingDocumentNoAfterGenerated(MLCOInvoiceWithholding iwh, MInvoice invoice,
		...
		String withhodingNo = "";
		boolean keep = MSysConfig.getBooleanValue("QSSLEC_FE_KEEP_WITHHOLDING_DOCNO_AFTER_GENERATED", false,
				Env.getAD_Client_ID(Env.getCtx()));
		if (!keep)
			return null;
		log.warning("QSSLEC_FE_KEEP_WITHHOLDING_DOCNO_AFTER_GENERATED = Y");
		if (newWithholding) {
