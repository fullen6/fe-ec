	...
	public String lecfeMovement_SriExportMovementXML100() {
		...
		try {
			...
			if (m_trxAutorizacionName == null) {
				StringBuilder l_trxname = new StringBuilder("SRI_").append(get_TableName()).append(get_ID());
			...
			/*
			 * if ( m_c_invoice_sus_id > 0) { // Numerico2 if (m_coddoc.equals("06"))
			 * addHeaderElement(mmDoc, "codDocSustento", "01", atts); // Hardcoded //
			 * Numerico15 -- Incluye guiones addHeaderElement(mmDoc, "numDocSustento",
			 * LEC_FE_Utils.formatDocNo(invsus.getDocumentNo(), "01"), atts); //
			 * Numerico10-37 if (asus.getSRI_AuthorisationCode() != null)
			 * addHeaderElement(mmDoc, "numAutDocSustento", asus.getSRI_AuthorisationCode(),
			 * atts); // Fecha8 ddmmaaaa addHeaderElement(mmDoc, "fechaEmisionDocSustento",
			...
			try {
				...
				while (rs.next()) {
					 ...
					 * 
					 * atts.clear(); atts.addAttribute("", "", "nombre", "CDATA", "descripcion1");
					 * atts.addAttribute("", "", "valor", "CDATA",
					 * LEC_FE_Utils.cutString(rs.getString(6),300)); mmDoc.startElement("", "",
					 * "detAdicional", atts); mmDoc.endElement("","","detAdicional");
					 * 
				...
				if (msg != null)
					if (msg.equals("RECIBIDA") || msg.contains("REGISTRADA")) {
						...
						if (autorizacionTrx != null) {
							...
							set_Value("SRI_Authorization_ID", null);
							saveEx();
