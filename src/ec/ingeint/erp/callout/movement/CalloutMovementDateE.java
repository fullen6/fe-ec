		...
		if (value != null) {
			...
			try {
				...
				if (datee.compareTo(dates) == -1) {
					...
					mTab.fireDataStatusEEvent("La fecha fin de entrega no puede ser menor a la fecha del inicio",
							sdates, true);
				}

				if (dates.compareTo(moved) == -1) {
					mTab.setValue("ShipDate", null);
					mTab.fireDataStatusEEvent("La fecha inicio de entrega no puede ser menor a la fecha del movimiento",
							movementdate, true);
				}
